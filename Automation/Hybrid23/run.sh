...
[[ "$signal47_hybrid_backend" == vulkan ]] || { echo 'This Hybrid23 runner currently permits only the reviewed Vulkan path.' >&2; exit 2; }
...
set +e
bash Automation/Splat21/bounded.sh 2147483648 120 /usr/bin/time -v -o "$signal47_hybrid_out/process-resources.txt" "$signal47_hybrid_player" -force-vulkan -screen-width 1280 -screen-height 800 -screen-fullscreen 0 --hybrid23-checks --hybrid23-out "$signal47_hybrid_out" -logFile "$signal47_hybrid_out/player.log" > "$signal47_hybrid_out/stdout.log" 2>&1
