...
if [[ ${1:-} == --checks ]]; then
  ...
  signal47_hybrid_args=(--hybrid23-checks --hybrid23-out "$signal47_hybrid_out" -logFile "$signal47_hybrid_out/player.log")
...
exec bash bounded.sh 2147483648 "$signal47_hybrid_seconds" "${signal47_hybrid_prefix[@]}" "$PWD/Hybrid23.x86_64" -force-vulkan -screen-width 1280 -screen-height 800 -screen-fullscreen 0 "${signal47_hybrid_args[@]}"
