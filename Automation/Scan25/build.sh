...
bash Automation/Splat21/bounded.sh 4294967296 600 "$signal47_hybrid_editor" -job-worker-count 2 -batchmode -nographics -quit -projectPath "$signal47_hybrid_project" -executeMethod Scan25Build.Build -logFile "$(realpath -m "$signal47_hybrid_log")"
python3 Automation/Scan25/identity.py "$signal47_hybrid_project" "${signal47_hybrid_log%.log}-identity.json"
