...
public static class Scan25Build
{
    ...
    static void EnsureRuntimeSettings()
    {
        ...
        Debug.Log("Scan25 runtime Resources settings and material/compute references persisted: "+path);
    }
    ...
    {
        var configPath = "Assets/Resources/Scan25Config.json";
        ...
        Debug.Log("Scan25 actual imported representation: "+asset.GetType().Name+" / "+asset.Compression+" / "+asset.SplatCount+" splats / "+importer.SourceCoordinates);
        ...
        feature.name = "Scan25 isolated UnitySplats"; AssetDatabase.AddObjectToAsset(feature, renderer); renderer.rendererFeatures.Add(feature);
        ...
        PlayerSettings.productName = "SIGNAL47 Scan25 Isolated Station";
        ...
        var stone = Material("Stone", new Color(.38f,.27f,.17f));
        ...
        var cam = new GameObject("Scan25 camera").AddComponent<Camera>(); cam.tag = "MainCamera";
        ...
        // The inherited project is Gamma. Unconditional conversion made trained colors dark/orange.
        // Preserve the project's color space; convert source sRGB only for a linear framebuffer.
        gs.GammaToLinear = PlayerSettings.colorSpace == ColorSpace.Linear;
        if(config.basis==null || config.basis.Length!=9 || config.synthetic || config.generatedMesh)
            throw new Exception("Scan25 requires an explicitly oriented, trained rock crop");
        // RUB import reflects source Z. This proper rotation supplies B * diag(1,1,-1).
        var localForward=new Vector3(-config.basis[2],-config.basis[5],-config.basis[8]);
        var localUp=new Vector3(config.basis[1],config.basis[4],config.basis[7]);
        var rockAnchor=new Vector3(6.2f,-.06f,2.0f);
        splats.transform.rotation=Quaternion.LookRotation(localForward,localUp);
        // sourceCenter is already in B-local RUF space (crop bounds), NOT raw PLY space.
        // R * importedPoint + translation = B * rawPoint + rockAnchor - sourceCenter.
        splats.transform.position=rockAnchor-config.sourceCenter;
        splats.transform.localScale=Vector3.one;
        if(asset.SHBands!=3)throw new Exception("Expected all three trained SH bands");
        // Conservative hand-authored solid interior: transparent splats have no topology.
        // This box is deliberately smaller than the scanned silhouette; it is not an extracted mesh.
        var rockCollider=Box("Rock solid interior / authored proxy",rockAnchor+new Vector3(0,.55f,0),new Vector3(1.7f,1.1f,1.0f),stone,ordinary);
        rockCollider.GetComponent<Renderer>().enabled=false;
        Box("Ruined foundation beneath scanned stone",rockAnchor+new Vector3(0,-.01f,0),new Vector3(3.25f,.12f,2.65f),stone,ordinary);
        ...
        var runtime = new GameObject("Scan25 runtime and checks").AddComponent<Scan25Runtime>();
        ...
        runtime.ordinaryRoot=ordinary.gameObject;
        runtime.rockCollider=rockCollider.GetComponent<Collider>();
        runtime.referenceViews=new[]{
            Pose("Reference front",rockAnchor+new Vector3(0,1.5f,-3.7f),rockAnchor+new Vector3(0,.7f,0)),
            Pose("Reference oblique",rockAnchor+new Vector3(3.4f,1.8f,-2.1f),rockAnchor+new Vector3(0,.7f,0)),
            Pose("Reference detail",rockAnchor+new Vector3(-.3f,1.25f,-1.75f),rockAnchor+new Vector3(-.3f,.7f,0))};
        runtime.photoPose=runtime.referenceViews[0];
        runtime.rockTestStart=Pose("Rock collision test",rockAnchor+new Vector3(0,0,-1.4f),rockAnchor+new Vector3(0,0,1));
        lamp.transform.position=rockAnchor+new Vector3(.9f,2,-1.8f);
        pole.transform.position=rockAnchor+new Vector3(.9f,1.1f,-1.8f);
        cyan.transform.position=rockAnchor+new Vector3(.55f,.65f,-1.25f);
        AssetDatabase.SaveAssets(); EditorSceneManager.SaveScene(scene,"Assets/Probe/Scan25.unity");
        var report=BuildPipeline.BuildPlayer(new BuildPlayerOptions{scenes=new[]{"Assets/Probe/Scan25.unity"},locationPathName="../Player/Scan25.x86_64",target=BuildTarget.StandaloneLinux64,options=BuildOptions.None});
        ...
        if(report.summary.result!=BuildResult.Succeeded)throw new Exception("Scan25 build failed");
    }
}
