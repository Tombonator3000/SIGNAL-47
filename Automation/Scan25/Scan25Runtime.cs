...
public sealed class Scan25Runtime : MonoBehaviour
{
    public Camera cam;
    ...
    public GameObject ordinaryRoot;
    public Transform[] referenceViews;
    public Transform rockTestStart;
    ...
    bool automated, benchmark, failed, doorOpen, capturing, initialized;
    double deadline;
    ...
    IEnumerator Start()
    {
        string[] args = Environment.GetCommandLineArgs(); benchmark=Array.IndexOf(args,"--scan25-benchmark")>=0; automated = benchmark || Array.IndexOf(args, "--scan25-checks") >= 0;
        int outIndex = Array.IndexOf(args, "--scan25-out");
        deadline = Time.realtimeSinceStartupAsDouble + (benchmark ? 170 : 90);
        try
        {
            if (outIndex >= 0 && (outIndex + 1 >= args.Length || args[outIndex + 1].StartsWith("--", StringComparison.Ordinal))) throw new ArgumentException("--scan25-out needs a separate output directory.");
            output = outIndex >= 0 ? Path.GetFullPath(args[outIndex + 1]) : Path.Combine(Application.persistentDataPath, "Scan25Diagnostics", DateTime.UtcNow.ToString("yyyyMMdd-HHmmss") + "-" + Guid.NewGuid().ToString("N").Substring(0, 8));
            Directory.CreateDirectory(output);
            if (File.Exists(Path.Combine(output, "result.json")) || Directory.GetFiles(output, "hybridprobe-exposure-*").Length > 0 || Directory.GetFiles(output, "*-frame-ms.csv").Length > 0) throw new IOException("Refusing to overwrite an existing Scan25 result or partial capture set.");
        }
        catch (Exception error) when (error is IOException || error is UnauthorizedAccessException || error is ArgumentException)
        ...
        else result.scope += " Trained-scan comparison: cropped Loop CE rock retains original learned positions, covariance, opacity and SH3. Ordinary scene stays visible in hybrid phases; reference views isolate the scan. No generated mesh or retraining is involved.";
        ...
        var scanRenderer=splatsRoot.GetComponent<GsplatRenderer>();
        result.colorSpace=QualitySettings.activeColorSpace.ToString();result.gammaToLinear=scanRenderer.GammaToLinear;
        Record("source-color-conversion-matches-project",scanRenderer.GammaToLinear==(QualitySettings.activeColorSpace==ColorSpace.Linear),"Actual project="+result.colorSpace+"; renderer GammaToLinear="+scanRenderer.GammaToLinear);
        ...
        if (!automated) { if(cyanOccluder)cyanOccluder.SetActive(false);SetPhotoView();Save(); yield break; }
        if(benchmark) { yield return Benchmark(); yield break; }
        yield return RunChecks();
    }
    IEnumerator Benchmark()
    {
        if(referenceViews==null || referenceViews.Length!=3 || !ordinaryRoot)
        { Abort("Missing benchmark route");yield break; }
        result.scope+=" Dedicated scripted camera sweep: 10-second warm-up and 60-second frame-interval sample, no screenshots during sampling. This is this small scene on this device, not full-game performance or native input.";
        ordinaryRoot.SetActive(true);SetRepresentation(true);lamp.enabled=true;
        photoPose=referenceViews[0];SetPhotoView();
        double warm=Time.realtimeSinceStartupAsDouble+10;
        while(Time.realtimeSinceStartupAsDouble<warm){yield return null;if(!Healthy())yield break;}
        var renderer=splatsRoot.GetComponent<GsplatRenderer>();
        bool ready=renderer && renderer.Valid && renderer.SplatCount==declaredSplatCount;
        Record("benchmark-trained-data-ready",ready,"All declared trained Gaussians uploaded before measurement.");
        if(!ready){Abort("Benchmark asset not ready");yield break;}
        var samples=new List<double>(15000);double start=Time.realtimeSinceStartupAsDouble,previous=start;
        player.enabled=false;
        while(Time.realtimeSinceStartupAsDouble-start<60)
        {
            double elapsed=Time.realtimeSinceStartupAsDouble-start;
            float fraction=(float)((Math.Sin(elapsed*Math.PI/15)+1)*.5);
            var from=referenceViews[2];var to=referenceViews[1];
            cam.transform.SetPositionAndRotation(Vector3.Lerp(from.position,to.position,fraction),Quaternion.Slerp(from.rotation,to.rotation,fraction));
        ...
        File.WriteAllLines(Path.Combine(output,"moving-rock-60s-frame-ms.csv"),samples.ConvertAll(v=>v.ToString("R",CultureInfo.InvariantCulture)));
        samples.Sort();double seconds=previous-start;
        result.phases.Add(new Phase{name="moving-rock-60s",representation=Representation,frames=samples.Count,seconds=seconds,
            meanMs=seconds*1000/samples.Count,p95Ms=samples[(int)((samples.Count-1)*.95)],p99Ms=samples[(int)((samples.Count-1)*.99)],
            splats=true,light=true,splatCount=declaredSplatCount,rendererCount=1,assetCount=declaredSplatCount,uploadedCount=renderer.SplatCount,assetsReady=ready});
        result.completed=true;if(!Save()){Abort("Benchmark evidence write failed");yield break;}Application.Quit(0);
    }
    void Update()
    {
    ...
    string Representation => splatsRoot.activeSelf ? (ordinaryRoot && ordinaryRoot.activeSelf ? "trained rock + ordinary scene" : "trained rock isolated / renderer comparison") : "ordinary scene only / trained rock disabled";
    public bool TryDoor()
    {
    ...
    void SetPhotoView()
    {
        ...
        yaw=player.transform.eulerAngles.y;pitch=cam.transform.localEulerAngles.x;
        if(pitch>180)pitch-=360;
    }
    IEnumerator RunChecks()
    {
        if(!ordinaryRoot || referenceViews==null || referenceViews.Length!=3 || !rockCollider || !rockTestStart)
        { Abort("Missing trained-rock reference/collision setup"); yield break; }
        var originalPhoto=photoPose;
        ordinaryRoot.SetActive(false);SetRepresentation(true);
        string[] names={"reference-front","reference-oblique","reference-detail"};
        for(int i=0;i<referenceViews.Length;i++)
        {
            photoPose=referenceViews[i];SetPhotoView();
            yield return Measure(names[i]); if(!Healthy())yield break;
            yield return CaptureScreen(names[i]); if(!Healthy())yield break;
            yield return CaptureExposure(names[i],false); if(!Healthy())yield break;
        }
        ordinaryRoot.SetActive(true);photoPose=originalPhoto;Physics.SyncTransforms();
        bool rockInPath=Physics.Raycast(rockTestStart.position+Vector3.up*.55f,rockTestStart.forward,out var rockHit,2f) && rockHit.collider==rockCollider;
        Record("authored-rock-proxy-in-path",rockInPath,"Conservative box inside the stone; not a mesh extracted from the scan.");
        yield return TestMove("rock-proxy-blocks-controller",rockTestStart,true);if(!Healthy())yield break;
        ...
        SetRepresentation(true);lamp.enabled=true;cyanOccluder.SetActive(false);
        yield return null;yield return null;if(!Healthy())yield break;
        yield return CaptureScreen("scene-preview");if(!Healthy())yield break;
        yield return CaptureExposure("scene-preview",false);if(!Healthy())yield break;
        cyanOccluder.SetActive(true);
        Record("separate-camera-exposures", result.exposures.Count == 8, "Three isolated views, four hybrid A/B states and a clean scene preview, through a separate disabled camera and URP SingleCameraRequest.");
    ...
    IEnumerator CaptureExposure(string name, bool compareLighting)
    {
        ...
        if (!exposureCamera)
        {
            exposureCamera = new GameObject("Scan25SeparateDiagnosticCamera").AddComponent<Camera>(); exposureCamera.enabled = false;
            exposureTarget = new RenderTexture(1280, 800, 24, RenderTextureFormat.ARGB32) { name = "Scan25DiagnosticExposure", antiAliasing = 1 };
