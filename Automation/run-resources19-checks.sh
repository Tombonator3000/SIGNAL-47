...
signal47_profile=$(mktemp -d "$PWD/Artifacts/Resources19/check-XXXXXX")
printf 'Disposable API test profile.\n' > "$signal47_profile/ALLOW_RESOURCES19_TEST"
printf '%s\n' "$signal47_profile" > Artifacts/Resources19/latest-profile.txt
signal47_player="${1:-$PWD/Artifacts/GauntletLinux/Signal47.x86_64}"
timeout --signal=TERM --kill-after=10s 180s "$signal47_player" --signal47-resources19-checks --signal47-save-dir "$signal47_profile" -force-glcore -screen-width 1280 -screen-height 800 -screen-fullscreen 0 -logFile "$signal47_profile/player.log" > "$signal47_profile/stdout.log" 2>&1
