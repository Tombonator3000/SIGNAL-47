...
namespace Signal47.Audio
{
    public static class ProceduralAudio
    {
        ...
        static float BoomSample(float t)=>t<0||t>1.25f?0:(Mathf.Sin(2*Mathf.PI*42*t)*Mathf.Exp(-2.2f*t)+Noise(t)*.18f*Mathf.Exp(-3*t))*.5f;
        static float SmashSample(float t)=>t<0||t>.55f?0:Noise(t)*Mathf.Exp(-7*t)*.42f+Mathf.Sin(2*Mathf.PI*(1450+2400*t)*t)*Mathf.Exp(-8*t)*.15f;
