...
## Environment27 additions

- **Boulder 01 — Rico Cilliers**, https://polyhaven.com/a/boulder_01, CC0-1.0, https://creativecommons.org/publicdomain/zero/1.0/. Powered by Poly Haven. The actual textured mesh and 2K diffuse/OpenGL normal/ARM maps are used, with Blender reductions to 3,306 and 396 triangles, a ground pivot, restrained material tint and Unity metallic/smoothness/AO channel packing. Raw metadata, original hashes and adaptation measurements are retained in `Art/Environment27/boulder01-source-license-manifest.json`. This is an ordinary mesh, not a Gaussian splat or a website preview image.
- The pinned five-file API URLs, SHA256 values and the reproducible Blender authoring recipe are recorded in `Docs/Environment27/BoulderSource.json` and `Automation/fetch-environment27-boulder.py`; the helper reuses verified source files and downloads only missing payloads.
- **Engine noise — Michael Brigida and students**, from *12 Ambient Machine Sounds*, https://opengameart.org/content/12-ambient-machine-sounds, CC BY 3.0, https://creativecommons.org/licenses/by/3.0/. Recorded by Berklee College of Music Advanced Sampling Class students under Associate Professor Michael Brigida; original editing by Diane Douglas and Colman O'Reilly; OLPC collection organized/prepared by Dr. Richard Boulanger; uploaded by bart. SIGNAL / 47 uses the previously retained Resources19 transcode, converted to mono 22.05 kHz PCM, with a 400 ms tail/head crossfade and DC removal. The gain is restrained and no endorsement is implied. Original source and output hashes are in `Art/Environment27/Audio/source-manifest.json`.

The station equipment, architectural modules and site-services plan are original project work. Existing Visual10 CC0 surfaces and adapted Grass Medium 02 geometry are reused; the same authored tufts now use shared GPU instances. Historical source assets and their notices are preserved.
